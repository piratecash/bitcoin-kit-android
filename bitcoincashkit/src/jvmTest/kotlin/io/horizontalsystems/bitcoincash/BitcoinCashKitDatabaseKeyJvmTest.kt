...
class BitcoinCashKitDatabaseKeyJvmTest {
    private val key = ByteArray(32) { it.toByte() }
    private val networkType = BitcoinCashKit.NetworkType.MainNet(MainNetBitcoinCash.CoinType.Type145)
    ...
    fun wordsConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            BitcoinCashKit(dir, key, OfflineConnectionManager, WORDS, "", WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    fun seedConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            BitcoinCashKit(dir, key, OfflineConnectionManager, SEED, WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    fun extendedKeyConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            BitcoinCashKit(dir, key, OfflineConnectionManager, HDExtendedKey(SEED, Purpose.BIP44), WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    fun watchAddressConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            BitcoinCashKit(dir, key, OfflineConnectionManager, ADDRESS, WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    private fun plaintextWalletDir(): String {
        val dir = Files.createTempDirectory("bitcoincash-key").toString()
        val database = CoreDatabase.getInstance(dir, BitcoinCashKit.getDatabaseName(networkType, WALLET, syncMode))
        try {
