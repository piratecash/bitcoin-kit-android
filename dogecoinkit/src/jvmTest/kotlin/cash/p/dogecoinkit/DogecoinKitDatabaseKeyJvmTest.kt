...
class DogecoinKitDatabaseKeyJvmTest {
    ...
    fun wordsConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            DogecoinKit(dir, key, OfflineConnectionManager, WORDS, "", WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    fun seedConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            DogecoinKit(dir, key, OfflineConnectionManager, SEED, WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    fun extendedKeyConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            DogecoinKit(dir, key, OfflineConnectionManager, HDExtendedKey(SEED, Purpose.BIP44), WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    fun watchAddressConstructor_keyOverPlaintextDatabase_requiresMigration() {
        ...
        assertThrows(DatabaseMigrationRequiredException::class.java) {
            DogecoinKit(dir, key, OfflineConnectionManager, ADDRESS, WALLET, networkType, syncMode = syncMode)
        }
    }
    ...
    private fun plaintextWalletDir(): String {
        val dir = Files.createTempDirectory("dogecoin-key").toString()
        val database = CoreDatabase.getInstance(dir, DogecoinKit.getDatabaseName(networkType, WALLET, syncMode))
        try {
    ...
    private companion object {
        const val WALLET = "w1"
        const val ADDRESS = "DTYVEuF3jzpExPeZixM3AZB8evwGp2Li4L"
